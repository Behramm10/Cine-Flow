...
-- Insert sample seats for different cinemas and auditoriums
INSERT INTO public.seats (cinema_id, auditorium, row_label, seat_number, seat_label) 
SELECT 
    c.id,
    'Screen 1',
    chr(ascii('A') + (gs.row_num - 1)),
    gs.seat_num,
    chr(ascii('A') + (gs.row_num - 1)) || gs.seat_num::text
FROM public.cinemas c
CROSS JOIN (
    SELECT 
        row_number() OVER (ORDER BY r, s) as row_num,
        s as seat_num
...
-- Insert more showtimes for tomorrow and day after
INSERT INTO public.showtimes (movie_id, cinema_id, auditorium, starts_at, base_price)
SELECT 
    m.id,
    c.id,
